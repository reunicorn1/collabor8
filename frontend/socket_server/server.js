...
async function handleStoreDocument(context, projectId) {
  try {
    const yMap = context.document.getMap('files');
    for (const [key, value] of yMap.entries()) {
      if (key.endsWith('_metadata')) {
        await updateDocumentInDb(context, projectId, key, value);
      }
    }
  } catch (error) {
...
async function updateDocumentInDb(context, projectId, key, metadata) {
  if (typeof metadata !== 'object' || metadata === null) {
    console.error('Metadata is not an object:', metadata);
    return;
  }
